...
rotate_dir = "/media/disk/01drive/06chengyang/14M/00"    # 保存路径
count = 0
...
for img_name in os.listdir(file_dir):
    ...
    # im_rotate = img.rotate(90)           # 指定逆时针旋转的角度
    # im_rotate = img.transpose(Image.FLIP_LEFT_RIGHT)
    save_name = img_name.split('/')[0].split('.')[0] + "_0.jpg"
    # print(rotate_dir + "/" + save_name)
    print(str(100*count/1371861)+"%  "+str(count))
    img.save(rotate_dir + "/" + save_name)  # 保存旋转后的图片
